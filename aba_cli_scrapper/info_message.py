...
def run_scrapper_with_success(folder_name: str):
    ...
    rprint("""[bold white]Here we go to the step :two:   [/bold white] initialize your database with [bold magenta blink]aba-run db-init  mysql --user your_database_username --password your_database_password --db-name alibaba_products[/bold magenta blink] command(for mysql engine) or [bold magenta blink]aba-run db-init --engine sqlite --sqlite-file sqlite_file_name(without any extensions) [/bold magenta blink] command(for sqlite engine)...""")
    # rprint(
